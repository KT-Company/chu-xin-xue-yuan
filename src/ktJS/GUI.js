...
function autoClip(formattedData) {
...
function container() {
  ...
  // orbitCamera&orbitControls
  {
    ...
    orbitCameraFolder
      .add(orbitControls, 'maxPolarAngle', 0, Math.PI / 2, 0.001)
      .name('maxPolarAngle')
      .onChange((val) => {
        CACHE.container.orbitControls.maxPolarAngle = val
      })
    orbitCameraFolder
      .add(orbitControls, 'minPolarAngle', 0, Math.PI / 2, 0.001)
      .name('minPolarAngle')
      .onChange((val) => {
    ...
    orbitCameraFolder
      .add(CACHE.container.bounds.boundingSphere, 'radius')
      .step(1)
      .name('radius')
      .onChange((val) => {
  ...
  // directionLights
  {
    ...
    lightFolder
      ...
      .name('平行光颜色')
    lightFolder
      .add(CACHE.container.directionLights[0].position, 'x')
      .onChange((val) => {
      ...
      .step(1)
    lightFolder
      .add(CACHE.container.directionLights[0].position, 'y')
      .onChange((val) => {
      ...
      .step(1)
    lightFolder
      .add(CACHE.container.directionLights[0].position, 'z')
      .onChange((val) => {
    ...
    const shadowCamera = CACHE.container.directionLights[0].shadow.camera
    const shadow = CACHE.container.directionLights[0].shadow
    ...
    lightFolder.add(state, 'mapSize', 0, 16384, 512).onChange(() => {
  ...
  //fog
  {
    ...
    const fogOptions = {
      type: CACHE.container.scene.fog ? (CACHE.container.scene.fog.isFogExp2 ? '指数雾' : '线性雾') : '无',
      color: CACHE.container.scene.fog ? CACHE.container.scene.fog.color.getHex() : 0x9dabb7,
      density: 0.0005, // 指数雾的默认密度
      near: 100, // 线性雾的默认近端
      far: 500, // 线性雾的默认远端
    }

    let colorControl, densityControl, nearControl, farControl

    // **更新 GUI 选项**
    const updateFogGUI = () => {
      if (colorControl) fogFolder.remove(colorControl)
      if (densityControl) fogFolder.remove(densityControl)
      if (nearControl) fogFolder.remove(nearControl)
      if (farControl) fogFolder.remove(farControl)

      if (fogOptions.type === '指数雾') {
        CACHE.container.scene.fog = new Bol3D.FogExp2(fogOptions.color, fogOptions.density)

        colorControl = fogFolder.addColor(fogOptions, 'color').onChange((value) => {
          CACHE.container.scene.fog.color.setHex(value)
        })

        densityControl = fogFolder
          .add(fogOptions, 'density', 0, 0.01)
          .step(0.00001)
          .onChange((value) => {
            CACHE.container.scene.fog.density = value
          })
      } else if (fogOptions.type === '线性雾') {
        CACHE.container.scene.fog = new Bol3D.Fog(fogOptions.color, fogOptions.near, fogOptions.far)

        colorControl = fogFolder.addColor(fogOptions, 'color').onChange((value) => {
          CACHE.container.scene.fog.color.setHex(value)
        })

        nearControl = fogFolder.add(fogOptions, 'near', 50, 2000).onChange((value) => {
          CACHE.container.scene.fog.near = value
        })

        farControl = fogFolder.add(fogOptions, 'far', 100, 5000).onChange((value) => {
          CACHE.container.scene.fog.far = value
        })
      } else {
        CACHE.container.scene.fog = null // 无雾
      }
    }

    // **添加 GUI 选择雾的类型**
    fogFolder.add(fogOptions, 'type', ['无', '指数雾', '线性雾']).onChange(() => {
      updateFogGUI()
    })
  ...
  // Bloom
  {
    ...
    bloomFolder
      .add(bloomParams, 'strength', 0.0, 10.0)
      .onChange((value) => {
  ...
  // transform
  {
    ...
    const options = {
      ...
      consoleLogPosition: () => {
        const position = Object.values(CACHE.container.orbitCamera.position).map((i) => Number(i.toFixed(2)))

        autoClip(position, 'position')
      },
      setTarget: () => {
        CACHE.container.orbitControls.target.copy(CACHE.container.transformControl.object.position)
      },
    }
    ...
    const step = 0.01
    let X, Y, Z, transformMode, LogTarget, LogPosition, setTarget
    transform.add(options, 'enable').onChange((val) => {
      if (!val) {
        ...
        transform.remove(Y)
        transform.remove(Z)
        transform.remove(LogTarget)
        transform.remove(LogPosition)
      } else {
        ...
        controls.attach(box)
        transformMode = transform.add(options, 'transformMode', options.transformMode).onChange((val) => controls.setMode(val))
        X = transform
          .add(box.position, 'x')
          .step(step)
        ...
        Z = transform
          ...
          .name('Z')
        LogTarget = transform.add(options, 'consoleLogTarget')
        LogPosition = transform.add(options, 'consoleLogPosition')
        setTarget = transform.add(options, 'setTarget')
      }
    })
  }
}
...
class modelAnimationGUI {
  ...
  updateModelFolder(model) {
    ...
    this.sceneAnimations[model].forEach((animation) => {
      const name = animation.clip.name
      const time = animation.clip.duration
      this.animationOptions[name] = {
        time: 0,
        play: false,
        helperArr: [],
      }
      const meshArr = animation.mixer._bindings.map((i) => i.binding.node)
      meshArr.forEach((i) => {
        const helper = new Bol3D.BoxHelper(i, 0xff0000)
      ...
      this.animationFolder
        .add(this.animationOptions[name], 'play')
        .name(`${name}-play`)
        .onChange((value) => {
          animation.mixer._actions[0].paused = !value
          const checkPlayAll = this.animationFolder.__controllers.filter((i) => i.property === 'play').every((i) => i.__checkbox.checked)
          this.modelFolder.__controllers.forEach((i) => i.property === 'playAll' && (i.__checkbox.checked = checkPlayAll) && (i.object.playAll = checkPlayAll))
        })
      this.animationFolder
        .add(this.animationOptions[name], 'time', 0, time, 0.1)
        .name(`${name}-time`)
        .onChange((value) => {
