...
import { onclick, ondblclick, onhover, onLoad, onProgress, Template } from './module'
import { STATE } from './STATE.js'

let container

// 原始加载方式
export function sceneOnLoad({ domElement, callback }) {
...
// 通过配置文件加载
export function loadSceneByJSON({ domElement, callback }) {
...
window.outCooroutCoordinate = () => {
  const camera = CACHE.container.orbitCamera.position
  console.log(
    'position: ',
    Object.values(camera).map((item) => Number(item.toFixed(2)))
  )
  const points = CACHE.container.orbitControls.target
  console.log(
    'target: ',
